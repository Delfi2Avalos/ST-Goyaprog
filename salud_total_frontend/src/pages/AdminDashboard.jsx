...
function AdminDashboard() {
  ...
  // Cargar turnos del admin como paciente
  ...
  useEffect(() => {
    if (activeTab === "turnos") cargarTurnos();
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => console.error("Error horas ocupadas"));
  }, [fecha, medico]);
  ...
  const bloquesDisponiblesAdmin = () => {
    ...
    horariosDia.forEach((h) =>
      bloques.push(...generarBloques(h.hora_inicio, h.hora_fin))
    );
  ...
  const cancelarTurno = async (turnoId) => {
    try {
      await api.patch(`/pacientes/turno/cancelar/${turnoId}/`);
              ...
              {pacientesChat.length === 0 ? (
                ...
                <ul>
                  {pacientesChat.map((chat) => (
                    ...
                    >
                      <strong>{chat.paciente_nombre}</strong>
                      <p>{chat.ultimo_mensaje || "Sin mensajes"}</p>
                    </li>
                  ))}
                </ul>
              )}
