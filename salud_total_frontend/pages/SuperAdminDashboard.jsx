...
function SuperAdminDashboard() {
  ...
  // Cargar especialidades hardcode
  useEffect(() => {
    setEspecialidades([
      { id: 1, nombre: "Clínica" },
  ...
  const registrarMedico = async () => {
    ...
    if (!horarios.length) {
      setMensaje("Debes agregar al menos un horario.");
      return;
    }
  ...
  const buscarFormularios = async () => {
    try {
      ...
      console.error("Error formularios:", err);
    }
  };
                  ...
                  {medicos.map((m) => (
                    <li key={m.id}>
                      ...
                      {pacientesAtendidos?.id === m.id && (
                        <> → {pacientesAtendidos.cantidad} pacientes</>
                      )}
                    </li>
                  ))}
                </ul>
